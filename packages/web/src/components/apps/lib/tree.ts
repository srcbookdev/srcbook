...
export class FileTree {
  ...
  private insertNode(dirNode: DirectoryNode, node: FileEntryType | DirectoryNode) {
    if (!dirNode.isAncestorOf(node)) {
      // As an optimization, we do not need to create copies of tree branches that are not
      // going to be affected by the insertion. We can reuse them as is (structural sharing).
      //
      // For example:
      //
      //  dirNode.path => /a/b
      //  node.path    => /a/c/d/index.tsx
      //
      // In this case, we can return `dirNode` as is since the `node` to insert is not a
      // descendent of `dirNode` and thus will not affect this branch of the tree.
      return dirNode;
    }
